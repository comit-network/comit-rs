...
use web3::{api::Namespace, helpers::CallFuture, types::U256, Transport};
...
impl<T: Transport> Ganache<T> {
    ...
    pub fn evm_snapshot(&self) -> CallFuture<SnapshotId, T::Out> {
    ...
    pub fn evm_revert(&self, id: &SnapshotId) -> CallFuture<bool, T::Out> {
        let id = web3::helpers::serialize(id);

        CallFuture::new(self.transport.execute("evm_revert", vec![id]))
    }
    ...
    pub fn evm_increase_time(&self, seconds: u64) -> CallFuture<u64, T::Out> {
        CallFuture::new(
            self.transport
    ...
    pub fn evm_mine(&self) -> CallFuture<String, T::Out> {
