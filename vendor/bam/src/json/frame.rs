...
//        self,
//        header::Header,
//        request::{UnknownMandatoryHeaders, UnvalidatedIncomingRequest,
// ValidatedIncomingRequest},    },
//};
...
impl Frame {
    pub fn new(frame_type: String, _: u32, payload: JsonValue) -> Self {
        Self {
            frame_type,
            payload,
        }
...
//            log::warn!(
//                "Sender of response future was unexpectedly dropped before
// response was received."            )
//        }))
//    }
//}
// impl JsonResponseSource {
//    pub fn get_awaiting_response(&mut self, id: u32) ->
// Option<Sender<json::Response>> {        self.awaiting_responses.remove(&id)
//    }
//}
...
//        &mut self,
//        frame: json::Frame,
//    ) -> Result<Option<Box<dyn Future<Item = json::Frame, Error = ()> + Send +
// 'static>>, Error>    {
...
// Box::new(future::ok(error_response)))                    .and_then(move
// |response| Ok(response.into_frame(frame_id)));
//
//                Ok(Some(Box::new(response_future)))
//            }
//            "RESPONSE" => {
//                let mut response_source =
// self.response_source.lock().unwrap();
//
...
//
//                log::debug!(
//                    "attempting to deserialize payload '{:?}' of frame {} as
...
//                match response {
//                    Ok(response) => {
//                        log::debug!("dispatching {:?} to stored handler",
// response);                        sender.send(response).unwrap()
//                    }
//                    Err(e) => log::error!(
...
//}
// fn unknown_mandatory_headers(unknown_headers: UnknownMandatoryHeaders) ->
// json::Response {    json::Response::new(Status::SE(1)).with_header(
//        "Unsupported-Headers",
...
//    ) -> Result<Box<dyn Future<Item = json::Response, Error = ()> + Send>,
