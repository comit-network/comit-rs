    ...
    fn inject_event(&mut self, event: comit_ln::BehaviourOutEvent) {
        match event {
            comit_ln::BehaviourOutEvent::SwapFinalized {
                ...
                //
                // match self.lnd_connector_params {
                //     None => { tracing::error!("Internal Failure: lnd
                // connectors are not initialised, no action has been taken.
                // This should be unreachable.") }     Some(ref
                // lnd_connector_params) => {         match role
                // {             Role::Alice => {
                //                 tokio::task::spawn({
                //                     let lnd_connector: LndConnectorAsReceiver
                // = (**lnd_connector_params).clone().into();
                //                     halight::new_halight_swap(local_swap_id,
                // params, self.halight_states.clone(), lnd_connector)
                //                         .instrument(
                //
                ...
                // create_swap_params.ethereum_identity;
                ...
                //                     let expiry =
                // create_swap_params.ethereum_absolute_expiry;
                //                     let token_contract =
                // create_swap_params.token_contract.into();
                //                     let erc20 = Erc20::new(token_contract,
                // asset);
                //
                ...
                // (**lnd_connector_params).clone().into();
                //
                // self::halight::new_halight_swap(local_swap_id, params,
                // self.halight_states.clone(), lnd_connector)
                //                         .instrument(
                //
                // tracing::error_span!("beta_ledger", swap_id = %local_swap_id,
                // role = %role),                         )
                ...
                //
                //                     let asset =
                // create_swap_params.ethereum_amount.clone();
                //                     let ledger = ledger::Ethereum::default();
                //                     let expiry =
                // create_swap_params.ethereum_absolute_expiry;
                //                     let token_contract =
