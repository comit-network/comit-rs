use crate::{identity, swap_protocols::SwapId};
...
const INFO: &str = "/comit/swap/ethereum/1.0.0";

/// The ethereum identity sharing protocol works in the following way:
///
/// - Dialer (Alice) writes the `Message` to the substream.
/// - Listener (Bob) reads the `Message` from the substream.

/// Data sent to peer in ethereum identity protocol.
#[derive(Clone, Copy, Deserialize, Debug, Serialize, PartialEq)]
pub struct Message {
    swap_id: SwapId,
    address: identity::Ethereum,
}

/// Represents a prototype for an upgrade to handle the sender side of the
/// ethereum identity sharing protocol.  Config contains the `Message`, once the
...
/// ethereum identity sharing protocol.
#[derive(Clone, Copy, Debug)]
...
mod tests {
    ...
    async fn correct_transfer() {
        let send_msg = Message {
            ...
            address: Address::from_str("8457037fcd80a8650c4692d7fcfc1d0a96b92867").unwrap(),
        };
