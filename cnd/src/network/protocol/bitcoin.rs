...
/// The bitcoin identity sharing protocol works in the following way:
///
/// - Dialer (Alice) writes the `Message` to the substream.
/// - Listener (Bob) reads the `Message` from the substream.

/// Data sent to peer in bitcoin identity protocol.
...
/// bitcoin identity sharing protocol.  Config contains the `Message`, once the
...
/// bitcoin identity sharing protocol.
#[derive(Clone, Copy, Debug)]
