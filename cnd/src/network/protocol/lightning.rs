...
const INFO: &str = "/comit/swap/bitcoin/1.0.0";

/// The lightning identity sharing protocol works in the following way:
///
/// - Dialer (Alice) writes the `Message` to the substream.
/// - Listener (Bob) reads the `Message` from the substream.

/// Data sent to peer in lightning identity protocol.
#[derive(Clone, Copy, Deserialize, Debug, Serialize, PartialEq)]
pub struct Message {
    swap_id: SwapId,
    pubkey: identity::Bitcoin,
}

/// Represents a prototype for an upgrade to handle the sender side of the
/// lightning identity sharing protocol.  Config contains the `Message`, once
/// the outbound upgrade is complete peer node has been sent the message.
...
/// lightning identity sharing protocol.
#[derive(Clone, Copy, Debug)]
