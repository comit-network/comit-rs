...
mod tests {
    ...
    fn comit_settings() -> Result<ConfigFile, config_rs::ConfigError> {
        ConfigFile::read("./config/cnd.toml")
    }
    ...
    fn config_folder_does_not_exist_will_create_folder_and_config_file() {
        ...
        let default_settings = ConfigFile::default(OsRng);

        let default_settings = default_settings.write_to(config_file_incl_path.clone());
        let settings = ConfigFile::read(config_file_incl_path.clone());
