...
export default class NectarActor implements Stoppable, DumpState, GetPeerId, GetListenAddress {
    ...
    public async assertBalancesChangedBy(expectedDiff: Partial<Balances>) {
        ...
        if (expectedDiff.bitcoin) {
            const expectedBalance = this.startingBalances.bitcoin + expectedDiff.bitcoin;
        ...
        if (expectedDiff.ether) {
            const expectedBalance = this.startingBalances.ether + expectedDiff.ether;
        ...
        if (expectedDiff.dai) {
            const expectedBalance = this.startingBalances.dai + expectedDiff.dai;
