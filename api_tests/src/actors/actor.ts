...
export class Actor {
    ...
    public async makeBtcDaiOrder(
        ...
        const rateInUsd = BigInt(price);
        const tenPow18 = BigInt("1000000000000000000");
        const rateInDai = rateInUsd * tenPow18;
        const dai = BigInt(sats) * rateInDai;
        const daiAsset = {
            ledger: LedgerKind.Ethereum,
            name: AssetKind.Erc20,
        ...
        const response = await this.cnd.client.post("/orders/btc/dai", {
            position,
            quantity: sats.toString(10),
            price: price * 10, // set price in cents,
            swap: {
