...
describe('RFC003 Bitcoin for Ether', () => {
    ...
    it("[Bob] Can execute the accept action", async () => {
        ...
        let bob_response = {
            ...
            alpha_ledger_success_identity: bob.wallet.btc_address().hash.toString('hex'),
            beta_ledger_lock_duration: 43200,
        };

        let accept_res = await chai.request(bob.comit_node_url())
            .post(accept_href)
            .send(bob_response);
    ...
    it("[Alice] Can get the HTLC fund action", async () => {
        let res = await chai.request(alice.comit_node_url()).get(alice_swap_href);
        res.should.have.status(200);
        res.body.state.should.equal('Accepted');
        res.body._links.should.have.property('fund');
    });
