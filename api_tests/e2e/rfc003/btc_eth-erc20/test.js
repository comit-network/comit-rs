...
describe("RFC003: Bitcoin for ERC20", () => {
    ...
    it(bob_initial_erc20 + " tokens were minted to Bob", async function() {
        ...
        let erc20_balance = await test_lib.erc20_balance(bob_wallet_address, token_contract_address);
    ...
    it("[Bob] Can get the deploy action from the ‘deploy’ link", async () => {
        ...
        logger.info("Bob retrieved the following deployment parameters", bob_deploy_action);
    });
    ...
    it("[Alice] Should have received the beta asset after the redeem", async function() {
        let alice_erc20_balance_after = await test_lib.erc20_balance(alice_final_address, token_contract_address);

        let alice_erc20_balance_expected = alice_erc20_balance_before.add(beta_asset_amount);
        alice_erc20_balance_after.toString().should.be.equal(alice_erc20_balance_expected.toString());
    });
