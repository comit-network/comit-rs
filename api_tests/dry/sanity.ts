...
setTimeout(async function() {
    oneActorTest("invalid-swap-yields-404", async function({ alice }) {
    ...
    oneActorTest("empty-swap-list-after-startup", async function({ alice }) {
    ...
    oneActorTest("bad-request-for-invalid-swap-combination", async function({
        alice,
    }) {
    ...
    oneActorTest("returns-invalid-body-for-bad-json", async function({
        alice,
    }) {
        const res = await request(alice.cndHttpApiUrl())
            .post("/swaps/rfc003")
    ...
    oneActorTest("alice-has-empty-peer-list", async function({ alice }) {
    ...
    oneActorTest("returns-listen-addresses-on-root-document", async function({
        alice,
    }) {
        const res = await request(alice.cndHttpApiUrl()).get("/");
    ...
    oneActorTest("can-fetch-root-document-as-siren", async function({ alice }) {
        const res = await request(alice.cndHttpApiUrl()).get("/");
