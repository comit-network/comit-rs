...
mod tests {
    ...
    ///
    /// We send money to an address and check if the transaction that we filter
    /// out is the same one as the one that was returned when we sent the
    /// money.
    #[tokio::test]
    async fn bitcoin_transaction_pattern_e2e_test() {
        ...
        let actual_transaction =
            tokio::time::timeout(Duration::from_secs(5), send_money_to_address)
                .await
