...
pub trait CurrentTime {
    // Two types that implement this trait do not necessarily support total
    // ordering. Where timestamps _do_ make up a total ordered set, the
    // timestamps generated by different implementations of `CurrentTime` can only
    // be constrained by partial ordering.
    //
    // What this means in practice is that the alpha expiry time and the beta expiry
    // time do not guarantee total ordering i.e., they can not be compared. We must
    // only compare durations not timestamps returned from different implementors of
    // this trait.
    //
