use crate::{asset, order::SwapProtocol, BtcDaiOrder, OrderId, Position};
use libp2p::PeerId;
...
mod tests {
    use super::*;
    use crate::{asset::bitcoin::btc, proptest};
    use spectral::prelude::*;
    use time::NumericalDuration;

    #[test]
    fn given_two_orders_with_same_price_then_should_match() {
        let buy = BtcDaiOrder::buy(btc(1.0), 9000, herc20_hbit());
        let sell = BtcDaiOrder::sell(btc(1.0), 9000, herc20_hbit());
    ...
    fn given_two_sell_orders_then_should_not_match() {
        let sell_1 = BtcDaiOrder::sell(btc(1.0), 9000, herc20_hbit());
        let sell_2 = BtcDaiOrder::sell(btc(1.0), 9000, herc20_hbit());
    ...
    fn given_sell_for_9000_when_buy_for_8500_then_no_match() {
        let sell = BtcDaiOrder::sell(btc(1.0), 9000, herc20_hbit());
        let buy = BtcDaiOrder::buy(btc(1.0), 8500, herc20_hbit());
    ...
    fn given_sell_for_8500_when_buy_for_9000_then_match_at_8500() {
        let sell = BtcDaiOrder::sell(btc(1.0), 8500, herc20_hbit());
        let buy = BtcDaiOrder::buy(btc(1.0), 9000, herc20_hbit());
    ...
    fn given_different_quantities_then_no_match() {
        let sell = BtcDaiOrder::sell(btc(0.5), 9000, herc20_hbit());
        let buy = BtcDaiOrder::buy(btc(1.0), 9000, herc20_hbit());
    ...
    fn given_different_swap_protocols_then_no_match() {
        let sell = BtcDaiOrder::sell(btc(1.0), 9000, herc20_hbit());
        let buy = BtcDaiOrder::buy(btc(1.0), 9000, hbit_herc20());
    ...
    fn given_same_swap_protocols_with_different_parameters_then_no_match() {
        let sell = BtcDaiOrder::sell(btc(1.0), 9000, SwapProtocol::HbitHerc20 {
            hbit_expiry_offset: 2.hours(),
            herc20_expiry_offset: 1.hours(),
        });
        let buy = BtcDaiOrder::buy(btc(1.0), 9000, SwapProtocol::HbitHerc20 {
    ...
    fn orderpool_does_not_emit_the_same_match_twice() {
        ...
        pool.publish(BtcDaiOrder::buy(btc(0.5), 9000, hbit_herc20()));
        ...
        assert_that(&matches_1).matching_contains(|m| m.price == 9000 && m.quantity == btc(0.5));
