...
impl Actions for bob::State<Ethereum, Bitcoin, EtherQuantity, BitcoinQuantity> {
    ...
    fn actions(&self) -> Vec<Action<Self::ActionKind>> {
        ...
        let mut actions = match (alpha_state, beta_state, self.secret) {
            ...
            (Funded { .. }, NotDeployed, _) => {
                vec![
                    bob::ActionKind::Fund(<(Bitcoin, BitcoinQuantity)>::fund_action(
        ...
        if let Funded { htlc_location, .. } = beta_state {
            actions.push(
                bob::ActionKind::Refund(<(Bitcoin, BitcoinQuantity)>::refund_action(
                    HtlcParams::new_beta_params(request, response),
                    htlc_location.clone(),
                    &*self.secret_source,
                ))
