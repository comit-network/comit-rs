...
pub fn state_store() -> HttpApiProblem {
    ...
    HttpApiProblem::with_title_and_type_from_status(HttpStatusCode::INTERNAL_SERVER_ERROR)
}
pub fn swap_not_found() -> HttpApiProblem {
    HttpApiProblem::new("swap-not-found").set_status(HttpStatusCode::NOT_FOUND)
}

pub fn unsupported() -> HttpApiProblem {
    HttpApiProblem::new("swap-not-supported").set_status(HttpStatusCode::BAD_REQUEST)
}

pub fn deserialize(e: &serde_json::Error) -> HttpApiProblem {
    error!("Failed to deserialize body: {:?}", e);
    HttpApiProblem::new("invalid-body")
        .set_status(HttpStatusCode::BAD_REQUEST)
...
pub fn action_already_done(action: Action) -> HttpApiProblem {
    ...
    HttpApiProblem::new("action-already-done").set_status(HttpStatusCode::INTERNAL_SERVER_ERROR)
}

pub fn invalid_action(action: Action) -> HttpApiProblem {
    error!("{} action is invalid for this swap", action);
    HttpApiProblem::new("invalid-action").set_status(HttpStatusCode::CONFLICT)
}
