...
fn given_deployed_htlc_when_redeemed_with_secret_then_money_is_transferred() {
    ...
    let snapshot_id = web3.api::<ganache_rust_web3::Ganache<web3::transports::Http>>()
        .evm_snapshot()
        .wait()
        .unwrap();
    ...
    web3.api::<ganache_rust_web3::Ganache<web3::transports::Http>>()
...
fn given_deployed_htlc_when_refunded_after_timeout_then_money_is_refunded() {
    ...
    let expiry_timestamp = EpochOffset::hours(1);

    let htlc = eth_htlc::Htlc::new(
        expiry_timestamp.clone(),
        refund_address,
        success_address,
        secret_hash,
    );
    ...
    let _ = web3.api::<ganache_rust_web3::Ganache<web3::transports::Http>>()
        .evm_increase_time(60 * 60 * 2)
        .wait()
        .unwrap();
    ...
    assert_eq!(
        refund_balance_before_htlc - receipt.gas_used + U256::from(htlc_value),
        refund_balance_after_htlc
    );
}
